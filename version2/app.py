...
def search_knowledge_base(knowledge_base, keyword):
    for doc in knowledge_base.split('\n'):
...
def handle_conversation(user_input, knowledge_base):
    ...
    if "don't remember" in user_input.lower() or "do not remember" in user_input.lower():
        if past_conversation:
            return f"That's okay! Let me help you. You talked about {keyword} when you were with [specific details]. Does that help jog your memory?"
        else:
            return "It's okay not to remember everything. Let's talk about something else."
    elif past_conversation:
        return f"I remember when you last mentioned {keyword}. We talked about {past_conversation}. How do you feel about that now?"
    else:
...
# Greeting the user first
if 'greeted' not in st.session_state:
    st.session_state.greeted = False

if not st.session_state.greeted:
    greeting_text = "Hello! I'm here to help you with your journaling today. When you're ready, you can start our conversation."
...
# Control flow with Start and Stop buttons
